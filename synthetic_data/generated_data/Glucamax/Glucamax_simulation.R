...
double CL = TVCL * pow(WT/80, WT_CL) * pow(CLCR/90, CLCR_CL) * 
            pow(AGE/55, AGE_CL) * exp(ETA_CL);
double V2 = TVV2 * pow(WT/80, WT_V2) * exp(ETA_V2);
double Q  = TVQ;
double V3 = TVV3;
...
if(covariates$SEX == 0) covariates$CLCR <- covariates$CLCR * 0.85  # Female adjustment
...
ex <- dosing_events %>%
  ...
  mutate(
    ...
    EXSTDTC = as.character(as.Date("2024-01-15") + days(time/24)),
    EXENDTC = EXSTDTC
  ) %>%
...
pc <- sim_df %>%
  ...
  mutate(
    ...
    PCSTRESU = "ng/mL",
    VISITNUM = ceiling(time / (7*24)) + 1,
    PCDTC = as.character(as.Date("2024-01-15") + days(time/24)),
    PCTPT = paste0(time %% 24, "H POST-DOSE"),
    PCELTM = time %% 24
  ) %>%
...
lb <- sim_df %>%
  ...
  mutate(
    ...
    LBSTRESU = "%",
    VISITNUM = ceiling(time / (7*24)) + 1,
    LBDTC = as.character(as.Date("2024-01-15") + days(time/24)),
