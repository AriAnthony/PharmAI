...
# Debug: Check PK data availability
pk_data_for_plot <- sim_df %>%
  filter(evid == 0, cmt == 2, time > 0, time <= 14*24)  # Extended to 2 weeks

cat("Debug: PK data summary for plotting:\n")
cat("Number of PK observations:", nrow(pk_data_for_plot), "\n")
cat("Time range:", min(pk_data_for_plot$time), "to", max(pk_data_for_plot$time), "hours\n")
cat("CP concentration range:", min(pk_data_for_plot$CP), "to", max(pk_data_for_plot$CP), "ng/mL\n")
cat("Number of subjects with PK data:", length(unique(pk_data_for_plot$ID)), "\n")
cat("Dose groups in PK data:", unique(pk_data_for_plot$DOSE), "\n")

# Generate enhanced PK plot with better visibility
pk_plot <- pk_data_for_plot %>%
  ggplot(aes(x = time, y = CP, color = factor(DOSE))) +
  geom_line(aes(group = interaction(ID, DOSE)), alpha = 0.5, size = 0.3) +
  stat_summary(fun = median, geom = "line", size = 1.5, alpha = 0.9) +
  stat_summary(fun = mean, geom = "point", size = 2, alpha = 0.8) +
  scale_y_log10(limits = c(0.1, NA)) +
  scale_x_continuous(breaks = seq(0, 14*24, by = 24)) +
  facet_wrap(~paste("Dose:", DOSE, "mg"), scales = "free_y") +
  labs(title = "Glucamax PK Profiles by Dose Group (First 2 Weeks)",
       subtitle = "Lines: Individual profiles, Thick lines: Median, Points: Mean",
...
pd_plot <- sim_df %>%
  ...
  geom_line(aes(group = interaction(ID, DOSE)), alpha = 0.4, size = 0.3) +
  stat_summary(fun = median, geom = "line", size = 1.5, alpha = 0.9) +
  stat_summary(fun = mean, geom = "point", size = 2, alpha = 0.8) +
...
# Ensure plots are displayed and saved
cat("\nDisplaying PK plot...\n")
print(pk_plot)
...
# Save plots to files
ggsave("pk_plot.png", pk_plot, width = 12, height = 8, dpi = 300)
ggsave("pd_plot.png", pd_plot, width = 10, height = 6, dpi = 300)

cat("\nPlots saved as pk_plot.png and pd_plot.png\n")
